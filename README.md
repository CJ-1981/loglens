...
![tests](https://img.shields.io/badge/assertions-378%20passing-green)
...
- **Current version**: v1.18.4 · 378 automated assertions across 8 suites (`test_loglens.js` + `test_loglens_v15.js` + `test_viewer_ui.js` + `test_viewer_search.js` + `test_team_nav.js` + `test_team_denoise.js` + `test_team_workbench.js` + `test_team_marks.js`) · worker-accelerated scans · zebra log viewer · mobile-responsive
...
- **v1.18.4 (current)** — **zebra striping in the viewer**: every other line gets a subtle translucent overlay for readability. Stripes are keyed to the printed line number's parity rather than CSS `:nth-child` — the viewer recycles rows while you scroll (edge chaining trims the buffer), so DOM-position striping would visibly slide; parity striping stays put and matches the gutter numbers. One translucent rule composites over the default theme, dark mode and all five logcat presets, and coexists with repeat-collapse dimming, the Δt column, search highlights and the focus ring. Alternation regression checks + real-browser checks in the default theme and under a preset
