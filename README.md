...
- **v1.14.3 (current)** — viewer tag column is **manually resizable**: drag the grip at the tag/message boundary (persisted; double-click returns to auto-fit), and a **wrap** toolbar toggle switches long lines between wrapped rows and one-line rows with horizontal scroll (persisted)
