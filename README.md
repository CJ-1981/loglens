...
- **Current version**: v1.21.4 · see changelog for the full history; unit suites run via `npm test`
...
- **v1.21.4 (current)** — viewer chain hardening after the v1.20 table refactor: the scroll-anchor, row-height and bookmark-center helpers still measured `#vBody.children` (the wrapper `<table>`, since rows became `<tr>` elements), which produced degenerate anchors and null dereferences in transient states — the `read error: Cannot read properties of null (reading 'length')` toast some viewer searches hit. They now query the real rows (`tr.vrow`); the edge chain sanity-checks its four window arrays and rebuilds the window instead of crashing; any chain failure persists its stack to `localStorage['loglens.lastReadErr']` for diagnosis; horizontal scroll is neutralized around measurements so wide nowrap rows cannot skew anchors
