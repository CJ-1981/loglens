...
- **v1.14.2 (current)** — viewer tag column auto-sizes to the longest tag in the current buffer (clamped 10–24ch) instead of reserving a fixed 16ch, so short tags no longer leave a blank stretch before the message while alignment is kept
