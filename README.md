...
![version](https://img.shields.io/badge/version-v1.22.15-blue)
![tests](https://img.shields.io/badge/assertions-483%20passing-green)
...
- **Current version**: v1.22.15 · see changelog for the full history; unit suites run via `npm test`
...
| **4 advanced** | sanitize copy · ai rule wizard (collapsed) | — |

The current step highlights as you scroll. After a run the results cards (scan overview, matches, export) appear between step 3 and the advanced section — they have no stepper entry because day-to-day browsing happens in the **viewer** tab. The advanced section appears expanded when relevant (the AI wizard opens itself when you generate or send to it).
...
Lives under **step 4 · advanced**. Describe what you want in plain language ("extract Bluetooth pairing failures, mask phone numbers, exclude HCI dumps") → a validated rule JSON comes back.
...
- **Deep scan (optional, opt-in)**: an enrichment pass for *residual* PII the regexes miss — free-text names, hostnames, addresses, whatever your rules don't cover. It samples the loaded files locally (worker-offloaded; error/warn/errish lines first, a reservoir over the rest), **masks every sampled line with your current rules before anything is sent**, then hands the masked sample to the engine you pick under **deep scan**:
  ...
  - **LLM** — reuses the AI wizard connection (step 4 · advanced; any OpenAI-compatible endpoint incl. Ollama/LM Studio). A dedicated system prompt asks for residual findings as JSON with suggested patterns; `parseDeepFindings` validates every suggested regex before it can become a rule.
...
- **v1.22.15 (current)** — **fixed: a narrow level filter (e.g. only W) made the viewer flicker uncontrollably**. With most levels hidden the rendered window can be shorter than a screenful; scrollTop then pins to 0 and the edge-chaining driver read *every* position as "at the bottom edge", chaining forward (or backward, if wheel-up intent was set) on every scroll event and interval tick — thousands of re-renders per stroke while the window slid through the file. Short content (scrollHeight ≤ viewport + 320px) now never auto-chains from the position heuristic: the wheel itself pages window-by-window on explicit intent, a fully-filtered stretch renders a hint row ("all lines in this stretch are hidden — wheel down for the next ones") instead of a fake end-of-file, and level-chip clicks reset the scroll-intent flags. Red/green-proven in Chromium: new E2E section 2b selects only the rare F level (added to the fixture at 1-per-500 lines) — idle mutation count 0, no drift, wheel pages forward on the fixed build; on the reverted build the first wheel burst chains to EOF and paging freezes
- **v1.22.14 (current)** — **AI wizard proxy works on current Chrome: `cors_proxy.js` now answers Chrome's Private/Local Network Access preflight**. From an https page (the live demo), calling the loopback proxy triggers a CORS preflight that Chrome requires to carry `Access-Control-Allow-Private-Network: true` — without it the browser blocks the request before the proxy is ever reached, so the "use local proxy" path still failed with `Failed to fetch` even while everything else was configured right. The relay now sends that header (plus an `Allow-Headers` echo and a 10-minute preflight max-age), and the wizard's failure diagnosis names the Local-Network-Access case (allow the site's local-network prompt in the address bar, or open LogLens from file://) instead of leaving it as a generic fetch error. Red/green-proven in Chromium with Private Network Access enforcement enabled: blocked without the header, `200` with it. **Note: restart `node cors_proxy.js` to pick up the fix — the old process keeps the old headers.** Also adds `docs/ARCHITECTURE.md`: current architecture + a simplification-redesign proposal (3 surfaces instead of 4 tabs, one unified search engine, progressive-disclosure gear menu)
- **v1.22.13** — **dark theme fixed — a lost `@media(max-width:640px){` opener had been silently breaking it since v1.22.6**: inserting the wide-monitor `@media(min-width:1800px)` block overwrote the mobile media query's opening line. The orphaned mobile rules became unconditional at every width (mask-rule table header hidden, run bar un-stickied, results table capped at 420px, 11px tables on desktop), and the stray closing brace that remained made Chromium's CSS parser swallow the very next rule — `body[data-theme=dark]{--bg:#0d1218;…}` — so dark mode rendered a light page and cards with dark-on-dark controls (selects, file items, rule rows). Restoring the opener fixes dark theme, the mask-rule header, the sticky run bar, the results-table height, and desktop font sizing in one line. New E2E section 26 asserts COMPUTED dark-theme colors (body `#0d1218`, cards `#151c24`, light select text) — the existing structural checks had stayed green through the entire breakage
- **v1.22.12** — **scroll anchors are byte-identity based — wheel scrolling now stays put with a level filter active**: v1.22.11 fixed the NaN snap for the unfiltered case, but with levels hidden the anchor still broke, because it was a *rendered-row index*: the level filter renders no row for hidden lines, so row indices and window-line indices diverge, and the chains' `a.idx − drop` / `a.idx + add` arithmetic restored the wrong row — the view jumped toward the top on every forward chain (exactly the "works only when all levels are active" report). `vCaptureAnchor` now captures the viewport row's `data-byte` (its stable line identity, already used by search/match walkers) plus the pixel offset; `vRestoreScroll` re-finds that byte after the re-render, falling back to the nearest row at-or-after when the anchor line itself is filtered out, and the chains need no index adjustment at all. Red/green-proven in Chromium: new E2E section 25b (wheel from file top with two levels hidden — trims happen, no reset, content never slides backward) FAILS on the old index anchor and PASSES with bytes
- **v1.22.11** — **root cause of the recurring "view snaps back to the top" found and fixed**: v1.14.5 added `anchor = Math.round(anchor || 0)` at the top of `vRender`, but edge chains (and every other anchored re-render — mask toggle, collapse toggle, tag auto-fit) pass the scroll anchor as a `{row, pixel}` object; `Math.round` coerced it to `NaN`, `vRestoreScroll` treated that as "no anchor", and every chained render reset `scrollTop` to 0. Wheeling downward through a window boundary therefore visibly threw the view back to the top — the instability behind several earlier "snaps to top" reports whose specific triggers got patched around it. Also fixed in passing: `vChainForward` measured the dropped lines' byte lengths *after* splicing the buffer, so `winStart` (footer %, byte offsets, row `data-byte` ids) drifted on logs with mixed line lengths. Regression-proven in Chromium (new E2E section 25: wheel from the file top through forward chains and trims — no hard reset, content never slides backward; the test was verified to FAIL against the deliberately re-broken build)
- **v1.22.10** — **jumping to a level-filtered line now warns instead of mis-aiming**: clicking a search-tab match whose level the viewer's level chips hide used to land silently on a *nearby* line (the filtered row is skipped at render time, so no focus ring appeared and the target looked wrong). Now the jump raises a warning toast — `match line N is level X — hidden by the viewer level filter · click to show X and re-jump` — and clicking it enables that level and re-jumps to the exact line. Bookmarks get the same guard (level read from the pinned line's header). `toast()` gained an optional one-click action (accent-outlined, hover-filled); verified end-to-end in Chromium (new E2E section 18b: warning shown, no wrong focus ring, one-click fix lands on the match line)
- **v1.22.9** — **one scroll bar in the viewer**: the byte rail moved from the left edge to the **right**, and the log body's native vertical scrollbar is hidden — the rail is now the single vertical scroll indicator, so position is never shown twice (and never disagreed, since the rail shows whole-file progress while the native bar only showed the buffered window). Wheeling over the rail scrolls the log (it sits where the native bar used to be), rail drag/click behavior is unchanged, and wide nowrap rows keep a horizontal scrollbar themed to match the rail (dark-theme track included). Verified in real Chromium on a 200k-line log: rail renders right of the body, `clientWidth` confirms zero native vertical gutter, wheel + drag + jump all move the view, no page errors
...
- **v1.19.4** — **viewer column header + mobile wrapped-width fix**. The log viewer gains a **column-names header** (`# · TIME · LVL · Δt · TAG · MESSAGE`) pinned above the log that mirrors the rows' exact column geometry — it follows the tag-column resize, appears/disappears with the Δt toggle, matches every logcat preset + dark theme, and syncs with horizontal scrolling. Also fixed: on narrow/mobile screens the wrapped **message column collapsed to ~1 character per line** — the fixed time/level/Δt/tag tracks consumed the whole width and the `word-break:break-all` message track shrank to its 1-char minimum; the message track is now bounded (`minmax(24ch,·)` desktop / `minmax(14ch,·)` mobile), so like the search-results table it keeps a usable width and the log scrolls horizontally when space runs out. Header alignment asserted to ±1px against the rendered columns + a 360px-viewport e2e width check
- **v1.19.3** — **search tab UI consistency pass, matched to the viewer**: the **mask** and **wrap** toggles now light up accent blue when active (same `on`-state styling as every viewer toggle — previously they gave no visual feedback), the results **column header flips between "text (masked)" and "text (raw)"** with the mask toggle, and **Enter in the pattern box runs the search**. Verified by computed-style screenshots across toggle states and surfaces + structural checks + accent/header e2e checks
- **v1.19.2** — **inline ✕ clear buttons on the text fields**: the search tab's pattern box, the search-results filter and the workbench results filter all gain the viewer-style ✕ (visible only while the field holds text; clearing refocuses the field and fires its normal input handling, so filters re-apply and validation borders reset). One shared `addInputClear()` helper wraps the input at wiring time and preserves its layout
- **v1.19.1** — **search tab display toggles, matching the viewer**: **mask on/off** (rows store the raw line — one string per row, same memory as before — and masking applies at display time, so the toggle flips instantly without rescanning; CSV/copy respect it, and the toggle defaults to masked with persistence), **wrap on/off** (one-line rows with horizontal scroll, persisted), and **zebra striping** in the results table using the same per-theme stripe colors as the viewer and the workbench results. Toggle checks in `test_viewer_ui.js` + e2e checks (masked↔raw text flip, nowrap switching, alternating row backgrounds)
- **v1.19.0** — **new "search" tab: regex search across all loaded files at once**. Streamed per file (any size), worker-offloaded like the other scanners with a main-thread fallback; results from every file land in one table (file · line · time · level · masked text) with a live filter, per-file match counts, CSV/copy export, and **click-through**: a row opens that file in the viewer, byte-seeks to the exact line and puts the match focus ring on it. **Memory-aware by design**: a "max results" cap (default 20,000) bounds stored rows with a per-file fair share so late files always contribute, while match *counts* keep running exactly past the cap (capped files are flagged, and a warning note states the stored total); an 8 MB newline valve plus progress/stop keep multi-GB loads safe. Driver-level behavioral tests (multi-file counts, caps, masking, case, abort) + wiring checks + real-browser e2e (two files, combined results, click-through, cap)
- **v1.18.10** — worker-scan fix + README screenshots. The worker scan driver omitted `bucketMin: 1`, so worker scans bucketed per `undefined` minute and the histogram's x-axis rendered **"12:NaN"-style labels** (and bar click-to-time-filter set a garbage window) — the in-page fallback was unaffected, which is why the node suites stayed green. Driver now pins `bucketMin: 1` like `processFile`; regression-guarded by a structural check and a strict e2e "no NaN histogram labels" check. README gained a Screenshots section (workbench light/dark, viewer HC/Dracula/Solarized, PII scan) captured from the real app
...
- **v1.18.5** — **zebra reaches the workbench results table + dark theme legibility pass**. The results table (`step 4 · matches`: time/lvl/tag/message/line) now stripes every other row too (append-only rows, so `nth-child` is stable there; `:not(.errrow):not(.ctxrow)` keeps the error/context tints on top), and the viewer stripe opacity was bumped for visibility. **Dark mode fixes**: active tabs and level/legend chips were white-text-on-near-white (`--ink` flips to light in dark) — now accent-filled; unclassed text inputs (time window from/to, err-ish regex, viewer go-to-time) rendered light-text-on-white — now dark-filled; verbose level chips, the truncation warning note and the mask-table header row had hardcoded light styles — moved to classes with dark variants; the histogram's hardcoded light SVG gridlines/text/bars gained dark counterparts. Browser-verified before/after (computed-color audit) + 6 structural regression checks + 3 e2e checks
- **v1.18.4** — **zebra striping in the viewer**: every other line gets a subtle translucent overlay for readability. Stripes are keyed to the printed line number's parity rather than CSS `:nth-child` — the viewer recycles rows while you scroll (edge chaining trims the buffer), so DOM-position striping would visibly slide; parity striping stays put and matches the gutter numbers. One translucent rule composites over the default theme, dark mode and all five logcat presets, and coexists with repeat-collapse dimming, the Δt column, search highlights and the focus ring. Alternation regression checks + real-browser checks in the default theme and under a preset
- **v1.18.3** — viewer follow-up to v1.18.2: the CSS `width:max-content` base still left the nowrap focus ring about **two characters short** of the rendered text — the flex→grid intrinsic sizing chain undersizes the row's max-content by a small constant that no pure-CSS rule reliably closes (tested: content-sizing on `.cols`, grid rows, floats, padding compensation). `vFitFocusRow()` now pins the focused row to its measured content extent (`scrollWidth + 1`, applied wherever `.mfocus` lands, cleared when it moves) on top of the CSS base; wrapped mode and short lines unchanged. The e2e geometry check now asserts against the **rendered right edge of the text** (widest content element) instead of `scrollWidth`, which had enough slack to hide the shortfall
- **v1.18.2** — viewer fix: with **wrap off** (one-line rows), the yellow search-match focus ring only spanned the visible viewport width while the matched line's text extended far to the right in the horizontal scroll — the focused row now expands to its full content width (`width:max-content;min-width:100%`), so the ring encloses the whole line; wrapped mode was and stays correct. CSS regression check in `test_viewer_ui.js` + a real-geometry e2e check (long line uploaded, nowrap, ring width ≥ text width)
