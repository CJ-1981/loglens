...
- **v1.21.3 (current)** — AI wizard test button: a missing `/models` route (404/405) no longer shows as an ambiguous warning; the test now falls back to a tiny 1-token chat completion probe and reports `✓ chat works — models route unavailable`, or the real chat error with status and message body
